...
async def process_natural_language_query(request: QueryRequest):
    ...
    try:
        ...
        response_data = {
            "success": True,
            "query": request.query,
            "answer": result.get("answer", result.get("response", "Query processed successfully")),
            "response": result.get("answer", result.get("response", "Query processed successfully")),  # Alternative key
            "classification": {
